...
def IDN(t_image, t_image_bicubic, scale, reuse=False):
    ...
    with tf.compat.v1.variable_scope("IDN", reuse=reuse):
        conv1 = tf.compat.v1.layers.conv2d(t_image, 64, (3, 3), (1, 1), padding='same', activation=lrelu, name='conv1')
        conv2 = tf.compat.v1.layers.conv2d(conv1, 64, (3, 3), (1, 1), padding='same', activation=lrelu, name='conv2')
        n = conv2
        for i in range(4):
...
def distillation(x, name=''):
    tmp = tf.compat.v1.layers.conv2d(x, 48, (3, 3), (1, 1), padding='same', activation=lrelu, name=name+'/conv1')
    tmp = GroupConv2d(tmp, act=lrelu, name=name+'/conv2')
    tmp = tf.compat.v1.layers.conv2d(tmp, 64, (3, 3), (1, 1), padding='same', activation=lrelu, name=name+'/conv3')
    tmp1, tmp2 = tf.split(axis=3, num_or_size_splits=[16, 48], value=tmp)
    tmp2 = tf.compat.v1.layers.conv2d(tmp2, 64, (3, 3), (1, 1), padding='same', activation=lrelu, name=name+'/conv4')
    tmp2 = GroupConv2d(tmp2, n_filter=48, act=lrelu, name=name+'/conv5')
    tmp2 = tf.compat.v1.layers.conv2d(tmp2, 80, (3, 3), (1, 1), padding='same', activation=lrelu, name=name+'/conv6')
    output = tf.concat(axis=3, values=[x, tmp1]) + tmp2
    output = tf.compat.v1.layers.conv2d(output, 64, (1, 1), (1, 1), padding='same', activation=lrelu, name=name+'/conv7')
    return output
...
def _phase_shift(I, r):
    return tf.compat.v1.depth_to_space(input=I, block_size=r)
...
def upsample(x, scale=4, features=32, name=None):
    with tf.compat.v1.variable_scope(name):
        x = tf.compat.v1.layers.conv2d(x, features, 3, padding='same')
        ps_features = 3 * (scale ** 2)
        x = tf.compat.v1.layers.conv2d(x, ps_features, 3, padding='same')
...
def GroupConv2d(x, n_filter=32, filter_size=(3, 3), strides=(1, 1), n_group=4, act=None, padding='SAME', name=None):
    groupConv = lambda i, k: tf.nn.conv2d(input=i, filters=k, strides=[1, strides[0], strides[1], 1], padding=padding)
    channels = int(x.get_shape()[-1])
    with tf.compat.v1.variable_scope(name):
        We = tf.compat.v1.get_variable(
